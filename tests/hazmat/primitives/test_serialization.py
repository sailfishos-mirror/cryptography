...
def _spki_subject_public_key(spki: bytes) -> bytes:
    """
    Returns the contents of the subjectPublicKey BIT STRING (excluding the
    unused bits byte) of a DER encoded SubjectPublicKeyInfo.
    """

    def parse_tlv(data: bytes, offset: int) -> tuple[int, bytes, int]:
        tag = data[offset]
        ...
        if length & 0x80:
            ...
            length = int.from_bytes(
                data[offset + 2 : offset + 2 + num_bytes], "big"
            )
            header_len += num_bytes
        start = offset + header_len
        return tag, data[start : start + length], start + length

    tag, content, end = parse_tlv(spki, 0)
    assert tag == 0x30 and end == len(spki)
    offset = 0
    while offset < len(content):
        tag, element, offset = parse_tlv(content, offset)
    assert tag == 0x03 and element[0] == 0
    return element[1:]
...
class TestDERSerialization:
    ...
    def test_load_pkcs8_private_key_v2_with_public_key(
        ...
        # The public key in the vector is the same as the subjectPublicKey
        # derived from the private key, and is the last element of the
        # structure.
        assert data.endswith(_spki_subject_public_key(public_bytes))
